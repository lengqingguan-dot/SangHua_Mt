...
function useSkill(skillId) {
    ...
    gameState.player.sp = Math.floor(currentSp - skill.cost); skill.effect();
    if (UI.elements.detailPanel) { UI.setDetail(renderSkillButtons()); }
    const updatedSp = gameState.player.sp || 0;
    print(`<span style="color: #aaffaa;">你的 HP: ${gameState.player.hp}/${gameState.player.maxHp} SP: ${updatedSp}/${gameState.player.maxSp||0}</span>`); print("");
}

function startMultiBattle(npcIds) {
    if (!npcIds || npcIds.length === 0) return;
    UI.setOverlay(true); if (typeof mainContent !== 'undefined' && mainContent) { UI.setOutputHtml(mainContent); }
    print(""); print(`═══════ ⚔️ 战斗开始 ═══════`);
    const enemies = [];
    npcIds.forEach((npcId, index) => { const npc = getCharacterInfo(npcId); if (npc && npc.canFight) { const enemyInstance = { index, npcId, name: npc.name, currentHp: npc.hp, maxHp: npc.hp, sp: npc.sp || 0, maxSp: npc.maxSp || 0, atk: npc.atk, def: npc.def, agi: getCharacterAgility(npc), drops: npc.drops ? [...npc.drops] : [], exp: npc.exp || 0 }; enemies.push(enemyInstance); print(`<span style="color: #ffaaaa;">敌人${index + 1}: ${npc.name} (HP:${npc.hp} ATK:${npc.atk} DEF:${npc.def} AGI:${enemyInstance.agi})</span>`); } });
...
function calculateTurnOrder() { const participants = [{ type: 'player', agi: getCharacterAgility(gameState.player) }]; battleState.enemies.forEach((enemy, index) => { if (enemy.currentHp > 0) participants.push({ type: 'enemy', index, agi: enemy.agi }); }); participants.sort((a, b) => a.agi !== b.agi ? b.agi - a.agi : (a.type === 'player' ? -1 : (b.type === 'player' ? 1 : Math.random() - 0.5))); return participants.map(p => p.type === 'player' ? 'player' : p.index); }
...
function startNewRound() {
    ...
    if (UI.elements.detailPanel) { UI.setDetail(renderSkillButtons()); } setTimeout(() => executeNextTurn(), 600);
}

function executeNextTurn() { if (!battleState.inBattle) return; if (battleState.enemies.filter(e => e.currentHp > 0).length === 0) { battleEnd(true); return; } if (gameState.player.hp <= 0) { battleEnd(false); return; } if (battleState.currentTurnIndex >= battleState.turnOrder.length) { battleState.round++; setTimeout(() => startNewRound(), 800); return; } const target = battleState.turnOrder[battleState.currentTurnIndex++]; target === 'player' ? executePlayerTurn() : executeEnemyTurn(target); }

function executePlayerTurn() {
    if (!battleState.inBattle || gameState.player.hp <= 0) { if (gameState.player.hp <= 0) battleEnd(false); return; }
    const aliveEnemies = battleState.enemies.filter(e => e.currentHp > 0); if (aliveEnemies.length === 0) { battleEnd(true); return; }
    const targetEnemy = aliveEnemies[0]; const playerAtk = getCharacterAttack(gameState.player);
    print(`<span style="color: #aaffaa;">→ 你的回合</span>`); print(`你选择攻击 ${targetEnemy.name}！`);
    ...
    if (UI.elements.detailPanel) { UI.setDetail(renderSkillButtons()); } setTimeout(() => executeNextTurn(), 1200);
}
...
function executeEnemyTurn(enemyIndex) {
    ...
    print(`<span style="color: #aaffaa;">你的 HP: ${gameState.player.hp}/${gameState.player.maxHp} SP: ${gameState.player.sp||0}/${gameState.player.maxSp||0}</span>`); setTimeout(() => executeNextTurn(), 1200);
}

function updateEnemyStatusDisplay(enemy) { print(`<span style="color: #ff8888;">${enemy.name} HP: ${enemy.currentHp}/${enemy.maxHp}</span>`); }
